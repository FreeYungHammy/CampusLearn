...
export const FileController = {
  ...
  myContent: async (req: AuthedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const items = await FileService.list({ tutorId: tutor._id }, 100, 0);
      res.json(items);
    } catch (e) {
      next(e);
    }
  ...
  getSignedUrlForVideo: async (req: AuthedRequest, res: Response, next: NextFunction) => {
    try {
      const { filename } = req.params;
      const signedUrl = await FileService.getSignedUrlForVideo(filename);
  ...
  byUser: async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const items = await FileService.list({ tutorId: userId as any }, 100, 0);
      res.json(items);
    } catch (e) {
      next(e);
    }
  ...
  byTutor: async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const items = await FileService.list({ tutorId: tutorId as any }, 100, 0);
      res.json(items);
    } catch (e) {
      next(e);
    }
    ...
    try {
      ...
      res.send(`
        <svg width="200" height="150" xmlns="http://www.w3.org/2000/svg">
          ...
          <text x="100" y="75" text-anchor="middle" fill="#666">Video Thumbnail</text>
        </svg>
      `);
    } catch (e) {
      next(e);
  ...
  update: async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const updated = await FileService.update(id, updates);
