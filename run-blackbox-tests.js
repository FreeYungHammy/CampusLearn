...
function recordTest(id, title, expected, actual, passed) {
  ...
  logger.log(`Actual: Status ${actual.status} - ${JSON.stringify(actual.body).substring(0, 100)}...`);
  logger.result(passed ? 'PASS' : 'FAIL', passed ? 'Test passed successfully' : 'Test failed - see details above');
}
...
async function runTests() {
  try {
    ...
    const registerRes = await makeRequest('POST', '/users/register', registerData);
    recordTest(
      'BB-AUTH-001',
      'Register New User',
    ...
    const invalidRes = await makeRequest('POST', '/users/register', invalidRegister);
    recordTest(
      'BB-AUTH-002',
    ...
    const threadData = {
      ...
      content: 'This is a test thread created during automated black-box testing',
      topic: 'Programming'
    };
    
    const createThreadRes = await makeRequest('POST', '/forum/threads', threadData, authToken);
    const threadPassed = createThreadRes.status === 201 && createThreadRes.body._id;
    ...
    const invalidThreadRes = await makeRequest('POST', '/forum/threads', invalidThread, authToken);
    recordTest(
      'BB-FORUM-002',
    ...
    const getThreadsRes = await makeRequest('GET', '/forum/threads', null, authToken);
    recordTest(
      'BB-FORUM-003',
      'Get Forum Threads',
      '200 OK with array of threads',
      getThreadsRes,
      getThreadsRes.status === 200 && Array.isArray(getThreadsRes.body.threads || getThreadsRes.body)
    );
    ...
    if (threadId) {
      logger.log('\nTest: Vote on Post');
      const voteRes = await makeRequest('POST', `/forum/threads/${threadId}/vote`, { voteType: 1 }, authToken);
      recordTest(
        'BB-FORUM-004',
        'Vote on Post',
    ...
    if (threadId) {
      ...
      const replyRes = await makeRequest('POST', `/forum/threads/${threadId}/replies`, replyData, authToken);
      recordTest(
        'BB-FORUM-005',
    ...
    const protectedRes = await makeRequest('GET', '/forum/threads', null, authToken);
    recordTest(
      'BB-AUTH-006',
    ...
    logger.log(`Passed: ${passedTests} (${Math.round(passedTests/totalTests*100)}%)`);
    logger.log(`Failed: ${failedTests} (${Math.round(failedTests/totalTests*100)}%)`);
    ...
    logger.log(`Environment: Development (localhost:5001)`);
...
logger.log('Make sure backend is running on http://localhost:5001\n');
