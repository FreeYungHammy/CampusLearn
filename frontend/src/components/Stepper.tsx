...
interface StepperProps {
  ...
  canProceedToNext?: (currentStep: number) => boolean;
  isNextButtonDisabled?: (currentStep: number) => boolean;
}
...
export default function Stepper({
  ...
  const updateStep = (newStep: number) => {
    if (externalCurrentStep !== undefined) {
      // If external currentStep is provided, just call onStepChange
          ...
          <div className={`footer-container ${footerClassName}`}>
            <div className={`footer-nav ${currentStep !== 1 ? 'spread' : 'end'}`}>
              {currentStep !== 1 && (
                <button
                  onClick={handleBack}
                  className={`btn-secondary ${currentStep === 1 ? 'inactive' : ''}`}
                  {...backButtonProps}
                >
                  {backButtonText}
                </button>
              ...
              <button 
                ...
                {...nextButtonProps}
              >
                {onCancel ? nextButtonText : (isLastStep ? nextButtonText : nextButtonText)}
              </button>
            </div>
          </div>
        )}
    ...
    <motion.div onClick={handleClick} className="step-indicator" animate={status} initial={false}>
      <motion.div
        variants={{
          ...
          active: { scale: 1, backgroundColor: '#3498db', color: '#3498db' },
          complete: { scale: 1, backgroundColor: '#3498db', color: '#2ecc71' }
        }}
...
function StepConnector({ isComplete }: { isComplete: boolean }) {
  const lineVariants = {
    ...
    complete: { width: '100%', backgroundColor: '#3498db' }
  };
